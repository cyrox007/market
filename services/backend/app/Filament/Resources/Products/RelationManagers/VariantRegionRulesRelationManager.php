...
class VariantRegionRulesRelationManager extends RelationManager
{
    protected static string $relationship = 'variantRegionRules';
    ...
    {
        // Показываем только для вариаций (торговых предложений)
        return $ownerRecord->isVariant();
    }
        ...
        return $schema
            ->components([
                ...
                TextInput::make('price_override')
                    ...
                    ->numeric()
                    ->prefix('₽')
                    ->helperText('Если указано, эта цена будет использоваться вместо базовой цены торгового предложения')
                    ->nullable(),
                ...
                Toggle::make('is_hidden')
                    ->label('Скрыть торговое предложение в локации')
                    ->default(false)
                    ->helperText('Если включено, торговое предложение будет скрыто в выбранной локации и всех её дочерних локациях. По умолчанию все товары показываются.')
                    ->columnSpanFull(),
            ...
            ->headerActions([
                CreateAction::make()
                    ...
                    ->mutateFormDataUsing(function (array $data): array {
                        ...
                        // Автоматически заполняем variant_id и product_id
                        $data['variant_id'] = $variant->id;
                        $data['product_id'] = $variant->parent_product_id;
