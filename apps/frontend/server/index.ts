    ...
    try {
      ...
      try {
        ...
        const parallelTasks: Promise<unknown>[] = [
          ...
          withCache(
            createCacheKey('/wishlist/count', undefined, userId === 'authenticated' ? 1 : undefined),
        ...
        if (hasAuthSession) {
          parallelTasks.unshift(
            withCache(
              createCacheKey('/auth/me', undefined, 1),
              () => ssrApi.auth.me(),
              { ttl: 60 },
            ),
          )
        ...
        if (hasAuthSession) {
          ...
          if (userResponse.status === 'fulfilled' && (userResponse.value as { user?: unknown })?.user) {
        ...
        if (regionResponse.status === 'fulfilled' && (regionResponse.value as { region?: unknown })?.region) {
        ...
        if (url === '/' || url === '') {
          try {
            ...
            const [categoriesResponse, featuredResponse, newResponse, saleResponse, slidersResponse, interiorIdeasResponse] = await Promise.allSettled([
              withCacheSWR(
        ...
        if (categoryMatch) {
          ...
          try {
            ...
            const priceMin = query.price_min ? parseInt(Array.isArray(query.price_min) ? query.price_min[0] : query.price_min, 10) : undefined
            const priceMax = query.price_max ? parseInt(Array.isArray(query.price_max) ? query.price_max[0] : query.price_max, 10) : undefined
            const sortBy = (Array.isArray(query.sort) ? query.sort[0] : query.sort) || 'created_at'
            const sortOrder = ((Array.isArray(query.order) ? query.order[0] : query.order) || 'desc') as 'asc' | 'desc'
            const colors = query.colors ? (Array.isArray(query.colors) ? query.colors : [query.colors]).flatMap(c => c.split(',').filter(Boolean)) : []
            const sizes = query.sizes ? (Array.isArray(query.sizes) ? query.sizes : [query.sizes]).flatMap(s => s.split(',').filter(Boolean)) : []
            ...
            Object.keys(query).forEach((key) => {
              if (key.startsWith('attr_')) {
                ...
                attributes[slug] = Array.isArray(value) ? value.flatMap(v => v.split(',').filter(Boolean)) : (value ? value.split(',').filter(Boolean) : [])
              }
            })
        ...
        if (searchMatch) {
          try {
            ...
            if (query) {
              const page = parseInt(Array.isArray(queryParams.page) ? queryParams.page[0] : queryParams.page || '1', 10)
              const sortBy = (Array.isArray(queryParams.sort) ? queryParams.sort[0] : queryParams.sort) || undefined
              const sortOrder = ((Array.isArray(queryParams.order) ? queryParams.order[0] : queryParams.order) || undefined) as
                | 'asc'
                | 'desc'
                | undefined
              const searchParams = { page, per_page: 24, sort_by: sortBy, sort_order: sortOrder, region_id: regionId }
      ...
      const pageSeo = ssrContext.product?.product?.seo ?? ssrContext.category?.category?.seo ?? null
      ...
      if (ssrContext.seo) {
        ...
        if (s.title) {
          template = template.replace(/<title>[\s\S]*?<\/title>/, `<title>${escapeHtml(s.title)}</title>`)
        }
        if (s.description) {
      ...
      const htmlWithState = template
        ...
        .replace(
          '</head>',
          `<script>window.__INITIAL_STATE__ = ${initialState}</script></head>`
        )
