...
export default function ProductStockBadge({
  ...
  if (!stockStatus.available && stockStatus.status === 'out_of_stock') {
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700 ${className}`}
      >
        Нет в наличии
      </span>
    );
  ...
  if (stockStatus.status === 'backorder') {
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 ${className}`}
      >
        Под заказ
      </span>
    );
  ...
  if (stockStatus.status === 'in_stock') {
    ...
    return (
      ...
      >
        {showStockCount && stockStatus.stock > 0 ? `В наличии (${stockText})` : 'В наличии'}
      </span>
    );
  }
  ...
  return (
    <span
      className={`px-2 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-700 ${className}`}
    >
      Проверить наличие
    </span>
  );
