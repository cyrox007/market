...
import type {
  ...
  ProductVariant,
  Review,
  /* VariationAttributeOption, */ SelectedVariationItem,
...
function getSpecificationsArray(
  specifications:
    ProductDetail['specifications'] | ProductVariant['specifications'] | null | undefined,
...
function productHasAnyDescription(product: ProductDetail): boolean {
  ...
  return (product.variants ?? []).some(
    (variant) =>
      hasMeaningfulProductText(variant.description) || hasMeaningfulProductText(variant.excerpt),
  );
}
...
export default function Product() {
    ...
    if (hasSSRProduct && initialProduct && (!slug || initialProduct.slug === slug) && !product) {
      ...
      if (sel?.length) {
        setSelectedVariation(
          sel.reduce((acc: Record<string, string>, s: SelectedVariationItem) => {
  ...
  useEffect(() => {
    ...
    const exactMatch = matching.find((v: ProductVariant) =>
      variantMatchesSelection(v, selectedVariation),
    );
  ...
  const productKey = slug ? getProductKey(slug, { region_id: regionId ?? undefined }) : null;

  const ssrProductPayload =
    hasSSRProduct && initialProduct && initialProduct.slug === slug
      ? {
          product: initialProduct,
          variant_info: initialVariantInfo,
          bundle: ssrData?.product?.bundle,
          related: ssrData?.product?.related,
        }
      : undefined;
  ...
  } = useSWR(productKey, () => api.products.get(slug!, { region_id: regionId ?? undefined }), {
  ...
  const { data: stockSettingsData } = useSWR('/api/stock-settings', () => api.stockSettings.get(), {
    revalidateOnFocus: false,
    dedupingInterval: 60_000,
  });
  ...
  const getVariationAttributesForCart = ():
    { attribute_slug: string; value_slug: string }[] | undefined => {
    ...
    if (!variant) {
      ...
      if (matching.length > 0) {
        ...
        if (bestVariant?.variation_attributes?.length) {
          return bestVariant.variation_attributes.map((a: SelectedVariationItem) => ({
    ...
    if (variant.variation_attributes?.length) {
      return variant.variation_attributes.map((a: SelectedVariationItem) => ({
        attribute_slug: a.attribute_slug,
        value_slug: a.value_slug,
      }));
    }
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      const msg =
        err?.data?.message || (err instanceof Error ? err.message : 'Ошибка добавления в корзину');
  ...
  const handleAddToCart = async () => {
    ...
    if (product.is_variable && !product.is_variant && product.variants) {
      ...
      if (selectedVariant) {
        isAvailable = selectedVariant.in_stock === true && (selectedVariant.stock ?? 0) > 0;
      } else {
        isAvailable = false;
      }
    }
  ...
  const handleSelectVariationAttribute = (
    ...
    setSelectedVariation((prev) => {
      const attrSlugs =
        product?.variation_attributes?.map((a: VariationAttributeOption) => a.attribute_slug) ?? [];
      ...
      for (const slug of attrSlugs) {
        ...
        const available =
          !currentSlug ||
          variantsWithThisValue.some((v: ProductVariant) =>
            v.variation_attributes?.some(
              (a: SelectedVariationItem) =>
                a.attribute_slug === slug && a.value_slug === currentSlug,
            ),
          );
        if (!available) {
          const first = variantsWithThisValue[0].variation_attributes?.find(
  ...
  const getSelectedVariant = (): ProductVariant | null => {
    ...
    const matching = inStockVariants.filter((v: ProductVariant) =>
      variantMatchesSelection(v, selectedVariation),
    );
    if (matching.length === 0) {
  ...
  const displayDescription =
    selectedVariant && hasMeaningfulProductText(selectedVariant.description)
      ...
      <div className="max-w-[1280px] mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-6 mb-12">
            ...
            <div className="flex flex-col flex-1 w-full md:max-w-[500px] order-1 md:order-2 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-2">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 border-2 rounded-lg py-2.5 cursor-pointer transition-colors ${
                    isInWishlist
                ...
                >
                  <Heart
                    className={`size-[1em] text-xl ${isInWishlist ? 'text-red-600' : 'text-red-600'}`}
                ...
                >
                  <ArrowLeftRight
                    className={`size-[1em] text-xl ${isInCompare ? 'text-red-600' : 'text-gray-700'}`}
                  />
                  <span className="text-sm font-medium">
            ...
            <div className="flex-1 space-y-4 sm:space-y-6 order-3 lg:order-3">
              ...
              <div>
                <div
                  ...
                  key={`status-${product?.id}-${selectedVariant?.id ?? 'parent'}-${displayNearestStock}`}
                >
                  {inStock ? (
                  ...
                  {(() => {
                    ...
                    const variantKey =
                      selectedVariant?.id ||
                      Object.values(selectedVariation).sort().join('-') ||
                      'main';
                    return displaySku ? (
                      <span
                        className="text-xs text-gray-500"
                        key={`sku-top-${displaySku}-${variantKey}`}
                      >
                        Арт: {displaySku}
                      </span>
                    ) : null;
                ...
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, i) => (
                      <Star
                        className="size-[1em] text-yellow-500 text-base sm:text-lg"
                  ...
                  {(() => {
                    ...
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        scrollToProductDetails('reviews');
                      }}
                      className="text-red-600 hover:underline text-xs sm:text-sm cursor-pointer"
                    >
                      {count} {label}
                    </a>;
                  })()}
              ...
              {product?.variation_attributes
                ...
                .map((attr) => (
                  <VariantAttributeSelector
                    ...
                    selectedValues={
                      selectedVariant
                        ? selectedVariation[attr.attribute_slug]
                          ? new Set([selectedVariation[attr.attribute_slug]])
                          : undefined
                        : undefined
                    }
                    ...
                    onSelect={(valueSlug, isDisabledClick) =>
                      handleSelectVariationAttribute(
                        attr.attribute_slug,
                        valueSlug,
                        isDisabledClick,
                      )
              ...
              {(() => {
                ...
                product.feature_blocks.length > 0 && (
                        ...
                        >
                          ...
                          >
                            {block.icon_image ? (
                              ...
                              <Icon
                                name={block.icon}
                                className="size-[1em] text-base sm:text-lg"
          ...
          <div className="bg-gray-50 rounded-2xl p-4 sm:p-6 h-fit lg:sticky lg:top-20 order-3 lg:order-3">
            ...
            <div
              className="mb-4"
              key={`stock-${product?.id}-${selectedVariant?.id ?? 'parent'}-${stockBadge.stock}-${stockBadge.stockText}`}
            >
              <div className="flex items-center gap-2 justify-center flex-wrap">
                <span
                  className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ${stockBadge.badgeClass}`}
                >
                ...
                {(() => {
                  ...
                  const variantKey =
                    ...
                    'main';
                  return displaySku ? (
                    <span className="text-xs text-gray-500" key={`sku-${displaySku}-${variantKey}`}>
                      Арт: {displaySku}
                    </span>
                  ) : null;
                })()}
                ...
                <div className="mt-4 pt-4 border-t border-gray-200">
                  <p className="text-sm text-gray-600">
                    ...
                    <span className="font-semibold text-red-600">
                      {formatPrice(bundleSetTotalPrice)}
                    </span>
                  </p>
                ...
                {(() => {
                    ...
                    if (selectedVariant) {
                      ...
                      if (inStockVariants.length > 0) {
                        ...
                        const compatible = inStockVariants.some((v: ProductVariant) => {
                          ...
                          for (const a of va) {
                            const sel = selectedVariation[a.attribute_slug];
                            if (sel !== undefined && sel !== '' && sel !== a.value_slug)
                              return false;
                          }
                          return true;
                        });
                          ...
                          <button
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                setAddToCartError(
                                  typeof msg === 'string' ? msg : 'Ошибка изменения количества',
                                );
                                setTimeout(() => setAddToCartError(null), 5000);
                              } finally {
                                setIsAddingToCart(false);
                  ...
                  return (
                    <button
                      ...
                      disabled={isAddingToCart || !isProductAvailable}
                      className={`py-3 rounded-lg font-semibold transition-all whitespace-nowrap shadow-lg disabled:opacity-50 disabled:cursor-not-allowed ${
                        !isProductAvailable
            ...
            <div className="flex gap-8 overflow-x-auto">
              {visibleTabs.map((tab) => (
                <button
                  ...
                  className={`flex items-center gap-2 pb-4 border-b-2 font-semibold cursor-pointer whitespace-nowrap transition-colors ${
                    activeTab === tab.id
          ...
          <div className="prose max-w-none">
            ...
            {activeTab === 'delivery' && (
              <div>
                ...
                product?.delivery_blocks &&
                Array.isArray(product.delivery_blocks) ? (
                      ...
                      <div key={review.id} className="border-b border-gray-200 pb-6">
                        <div className="flex items-center justify-between mb-3">
                          ...
                          <div className="flex gap-1">
                            {[...Array(5)].map((_, i) => (
                              <Star
                                className="size-[1em] text-yellow-500 text-lg"
