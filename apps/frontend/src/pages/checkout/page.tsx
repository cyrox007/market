...
const normalizeAddressValue = (value?: string | null): string => (value ?? '').trim().toLowerCase();
...
export default function Checkout() {
  ...
  const shippingMethods = useMemo(() => {
    ...
    return shippingMethodsRaw.map((method) => {
      ...
      let price =
        method.price !== undefined && method.price !== null
          ? method.price
          : method.base_price !== undefined && method.base_price !== null
            ? method.base_price
            : 0;

      // Проверяем порог бесплатной доставки (если цена еще не была обнулена на бэкенде)
      if (
        method.free_delivery_threshold &&
        cart?.subtotal &&
        cart.subtotal >= method.free_delivery_threshold
      ) {
        price = 0;
      }
  ...
  const loadShippingOptions = useCallback(async () => {
    ...
    try {
      const [methodsResponse, handlingTypesResponse, additionalServicesResponse]: any =
        await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedAdditionalServices.size > 0) {
        orderData.additional_services = Array.from(selectedAdditionalServices.entries()).map(
      ...
      if (user && response.order) {
        ...
        const addressNeedsUpdate =
          hasAddress &&
          (!selectedAddress ||
            normalizeAddressValue(selectedAddress.city) !==
              normalizeAddressValue(addressForm.city) ||
            normalizeAddressValue(selectedAddress.street) !==
              normalizeAddressValue(addressForm.street) ||
            normalizeAddressValue(selectedAddress.house) !==
              normalizeAddressValue(addressForm.house) ||
            normalizeAddressValue(selectedAddress.apartment) !==
              normalizeAddressValue(addressForm.apartment) ||
            normalizeAddressValue(selectedAddress.entrance) !==
              normalizeAddressValue(addressForm.entrance) ||
      ...
      if (response.order?.id && response.gateway_client_config) {
        try {
          ...
          if (!paymentOpened) {
            const msg =
              'Заказ создан. Разрешите всплывающие окна или нажмите «Оплатить» на странице заказа.';
          ...
          const msg =
            'Заказ создан, но не удалось открыть оплату. Нажмите «Оплатить» на странице заказа.';
          showPaymentTabError(paymentTab, msg);
          closePaymentTab(paymentTab);
          setError(msg);
          window.scrollTo({ top: 0, behavior: 'smooth' });
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-2 text-sm mb-2">
          ...
          <Link to="/cart" className="text-gray-600 hover:text-red-600">
            Корзина
          </Link>
          <ChevronRight className="size-[1em] text-gray-400" />
          ...
          <div className="flex items-center justify-between max-w-3xl mx-auto">
            {[
              ...
              <div key={idx} className="flex items-center flex-1">
                <div className="flex flex-col items-center flex-1">
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center font-bold mb-2 ${
                      item.active ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-500'
                    }`}
                  >
                    {item.step}
                  </div>
                  <span
                    className={`text-sm font-medium ${item.active ? 'text-red-600' : 'text-gray-500'}`}
                  >
                    {item.label}
                  </span>
                </div>
                {idx < 3 && (
                  <div
                    className={`h-1 flex-1 mx-4 ${item.active ? 'bg-red-600' : 'bg-gray-200'}`}
                  ></div>
                )}
              </div>
            ))}
                  ...
                  <div className="space-y-4">
                    ...
                    {savedAddresses.length > 0 && (
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            handleAddressSelect(e.target.value ? parseInt(e.target.value) : null)
                          }
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg text-sm focus:border-red-600 focus:outline-none"
                        >
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-gray-500">
                        Выбор локации меняет регион на всём сайте: цены и наличие товаров зависят от
                        региона.
                      </p>
                    </div>
                    <div>
                              ...
                              <div className="flex-1">
                                ...
                                {method.free_delivery_threshold && method.price > 0 && (
                                  <p className="text-xs text-gray-500 mt-1">
                                    ...
                                    {method.free_delivery_threshold.toLocaleString()} ₽
                                  </p>
                                )}
                              </div>
                            </label>
                          ...
                          {deliveryHandlingTypes.map((type) => (
                            <div
                              ...
                              className={`p-4 border-2 rounded-lg transition-all ${
                                deliveryHandlingTypeId === type.id
                                  ? 'border-red-600 bg-red-50'
                                  : 'border-gray-200'
                              }`}
                            >
                                  ...
                                  {type.requires_floor && deliveryHandlingTypeId === type.id && (
                                    <div className="mt-3">
                                      ...
                                      <input
                                        ...
                                        value={deliveryFloor || ''}
                                        onChange={(e) =>
                                          setDeliveryFloor(parseInt(e.target.value) || null)
                                        }
                                      ...
                                      />
                                      {type.example_prices &&
                                        Object.keys(type.example_prices).length > 0 && (
                                          <p className="text-xs text-gray-500 mt-1">
                                            Пример:{' '}
                                            {Object.entries(type.example_prices)
                                              .slice(0, 3)
                                              .map(([floor, price]) => (
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-8">
                ...
                {additionalServices.length === 0 ? (
                  <p className="text-gray-500 text-center py-8">
                    Нет доступных дополнительных услуг для выбранной локации
                  </p>
                ) : (
                  <div className="space-y-4">
                    {additionalServices.map((service) => {
                      ...
                      const servicePrice =
                        selectedAdditionalServices.get(service.id)?.price ?? service.price;
                            ...
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center gap-3">
                                {service.icon && (
                                  <Icon
                                    name={service.icon}
                                    className="size-[1em] text-2xl text-red-600"
                                  />
                                )}
                              ...
                              <div className="flex flex-col items-end">
                                {service.price_type === 'custom' ? (
                                  ...
                                  <span
                                    className={`font-bold ${servicePrice === 0 ? 'text-green-600' : 'text-gray-700'}`}
                                  >
                                    {servicePrice === 0
                                      ? 'Бесплатно'
                                      : servicePrice
              ...
              <div className="bg-white border border-gray-200 rounded-2xl p-8">
                ...
                <div className="space-y-4">
                  {paymentMethods.map((method) => (
                    <label
                      key={method.id}
                      className={`flex items-start gap-4 p-4 border-2 rounded-lg cursor-pointer hover:border-red-600 transition-colors ${
                        paymentMethod === method.code ? 'border-red-600' : ''
                      }`}
                    >
                      <input
                        type="radio"
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          {method.icon && (
                            <Icon name={method.icon} className="size-[1em] text-2xl text-red-600" />
                          )}
                ...
                <div className="border-t border-gray-200 pt-4 space-y-3 mb-6">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {cart?.subtotal ? cart.subtotal.toLocaleString() : '0'} ₽
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Доставка</span>
                    <span
                      className={`font-medium ${deliveryCost === 0 && deliveryCostKnown ? 'text-green-600' : ''}`}
                    >
                      {isCalculating
                        ? 'Расчёт…'
                        : !deliveryCostKnown
                  ...
                  {selectedAdditionalServices.size > 0 && (
                    <div className="flex flex-col gap-2">
                      {Array.from(selectedAdditionalServices.entries()).map(
                        ([serviceId, serviceData]) => {
                          const service = additionalServices.find((s) => s.id === serviceId);
