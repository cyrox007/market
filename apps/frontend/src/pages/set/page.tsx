				...
				<div className="grid grid-cols-[1fr_300px] gap-8 mb-12">
									...
									<div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
										{allImages.map((_, idx) => (
											<button
												...
												className={`w-3 h-3 rounded-full transition-all ${selectedImage === idx ? 'bg-red-600 w-8' : 'bg-white/60'
													}`}
											/>
										))}
									</div>
													...
													>
														...
														<div className="flex gap-2">
															<button
																...
																className={`py-2 rounded-lg font-medium transition-all whitespace-nowrap cursor-pointer ${cartItems.includes(item.id)
								...
								<div className="flex gap-3 overflow-x-auto pb-2">
									{allImages.map((img, idx) => (
										<button
											key={idx}
											onClick={() => setSelectedImage(idx)}
											className={`flex-shrink-0 w-20 h-16 rounded-lg overflow-hidden border-2 transition-all cursor-pointer ${selectedImage === idx ? 'border-red-600 scale-105' : 'border-gray-200 hover:border-gray-300'
												}`}
										>
											{img ? (
												<img
					...
					<div className="bg-gray-50 rounded-2xl p-6 h-fit sticky top-20">
						<div className="flex items-center gap-2 mb-3">
							<span className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap ${set.inStock ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
								}`}>
						...
						<div className="space-y-3 mb-6">
							<div className="flex gap-2">
								<button
									...
									className={`py-3 rounded-lg font-semibold transition-all whitespace-nowrap shadow-lg ${addedToCart
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5" data-product-shop>
						{set.items.map((item) => {
							...
							return (
								<div
									key={item.id}
									className={`bg-white border-2 rounded-2xl overflow-hidden transition-all ${isInActiveSet
									...
									<div className="p-4">
										...
										{isInActiveSet ? (
											...
											<button
												...
												className={`w-full py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${cartItems.includes(item.id)
