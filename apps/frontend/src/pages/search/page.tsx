...
export default function Search() {
	...
	const swrKey = searchParamsForSWR ? getSearchKey(searchParamsForSWR.query, searchParamsForSWR) : null;
				...
				<div className="mb-6 md:mb-8">
					...
					{query && total > 0 && (
						<p className="text-gray-600 text-sm md:text-base mt-4">
							По запросу <span className="font-semibold text-gray-900">"{query}"</span> найдено: <span className="font-semibold">{total}</span> {total === 1 ? 'товар' : total < 5 ? 'товара' : 'товаров'}
						</p>
					)}
					{query && total === 0 && !isLoading && (
						<p className="text-gray-600 text-sm md:text-base mt-4">
							По запросу <span className="font-semibold text-gray-900">"{query}"</span> ничего не найдено
						</p>
					)}
				</div>
								...
								{(() => {
									...
									return pages.map((page) => (
										<button
											...
											className={`w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-lg font-medium cursor-pointer text-sm md:text-base ${currentPage === page ? 'bg-red-600 text-white' : 'border border-gray-300 hover:border-red-600'
												}`}
										>
											{page}
										</button>
