...
import { ArrowLeftRight, ChevronRight, ImageIcon, Info, LoaderCircle, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            <div className="w-32 h-32 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <ArrowLeftRight className="size-[1em] text-6xl text-gray-400" />
            </div>
            ...
            <Link
              to="/catalog"
              className="inline-block bg-red-600 text-white px-8 py-3 rounded-lg font-medium hover:bg-red-700 transition-colors whitespace-nowrap cursor-pointer"
            >
              Перейти в каталог
            </Link>
          </div>
                  ...
                  {products.map((product) => (
                    ...
                    >
                      <div className="relative">
                        ...
                        >
                          <X className="size-[1em] text-white text-sm" />
                        </button>
                        <div className="relative">
                        ...
                        <h3 className="font-bold text-sm mb-2 min-h-[40px] line-clamp-2">
                          {product.name}
                        </h3>
                ...
                {allSpecifications.length > 0 ? (
                  allSpecifications.map((specKey) => (
                    ...
                    >
                      ...
                      {products.map((product) => (
                        <div
                          key={product.id}
                          className="w-64 flex-shrink-0 p-4 border-r border-gray-200 last:border-r-0 text-sm"
                        >
                  ...
                  <div className="flex border-b border-gray-200">
                    ...
                    {products.map((product) => (
                      <div
                        key={product.id}
                        className="w-64 flex-shrink-0 p-4 border-r border-gray-200 last:border-r-0 text-sm text-gray-500"
                      >
                        Не указаны
                      </div>
                    ))}
        ...
        {products.length > 0 && (
          <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
            <Info className="size-[1em] text-yellow-600 text-xl flex-shrink-0 mt-0.5" />
            <div className="text-sm text-gray-700">
              <p className="font-medium mb-1">Подсказка:</p>
              <p>
                Прокрутите таблицу влево, чтобы увидеть все товары. Прокрутите вниз, чтобы сравнить
                все характеристики.
              </p>
            </div>
          </div>
